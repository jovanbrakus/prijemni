...
async function processLesson(lessonHtmlPath) {
  let html = fs.readFileSync(lessonHtmlPath, "utf-8");

  // Extract generation prompt from KNOWLEDGE_LESSON_META
  ...
  const heroFilename = path.basename(outFile);

  console.log(`Saved: ${outFile} (${(buffer.length / 1024).toFixed(0)} KB)`);

  // Update HTML: replace unsplash src with local filename
  html = html.replace(
    /src="https:\/\/images\.unsplash\.com\/[^"]+"/,
    `src="${heroFilename}"`
  );

  // Remove overlay-chips div (keep overlay-note with margin-top:auto)
  html = html.replace(
    /\s*<div class="overlay-chips">[\s\S]*?<\/div>\s*(?=\s*<div class="overlay-note")/,
    "\n"
  );

  // Add margin-top:auto to overlay-note if not already present
  html = html.replace(
    /<div class="overlay-note">/,
    '<div class="overlay-note" style="margin-top:auto">'
  );

  // Update current_src in metadata
  html = html.replace(
    /"current_src":\s*"[^"]+"/,
    `"current_src": "${heroFilename}"`
  );

  fs.writeFileSync(lessonHtmlPath, html);
  console.log(`Updated HTML: ${lessonHtmlPath}\n`);
}

const target = process.argv[2];
if (!target) {
  console.error("Usage: node scripts/process-hero-image.mjs <lesson.html>");
  process.exit(1);
}

processLesson(path.resolve(target));
