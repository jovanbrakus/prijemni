...
export function Sidebar({
  ...
  const [mode, setMode] = useState<"v1" | "v2">(
    selectedProblemId?.startsWith("v2_") ? "v2" : "v1"
  );

  const activeFaculties = mode === "v2" ? facultiesV2 : faculties;
  ...
  // Total reports in the active mode
  const activeReportCount = Array.from(facultyReportCounts.values()).reduce((a, b) => a + b, 0);
  ...
  return (
    <ScrollArea className="h-full">
      <div className="p-3">
        ...
        {/* Mode toggle */}
        <div className="mx-1 my-2 flex rounded-lg bg-background p-0.5">
          <button
            onClick={() => setMode("v1")}
            className={cn(
